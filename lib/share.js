...
// Dots delimit the code and the name segment of decodeCode's pattern accepts
// neither dots nor whitespace — so both must collapse to something safe, NOT
// to a space (that produced codes nobody could decode: any Epic display name
// with a space in it, e.g. "Dark Knight 99", failed outright until
// 2026-07-26). Underscore keeps names readable in a URL too.
...
// Everything you have to trade with = mastered + found. Pass extra manual
// keys (the user's on-device toggles) to fold them in.
export function ownedKeySet(collection, extra) {
  const set = new Set(extra || []);
  for (const bucket of [collection?.mastered, collection?.found])
    for (const [slug, vs] of Object.entries(bucket || {}))
      for (const variant of Object.keys(vs)) set.add(`${slug}:${variant}`);
  return set;
}
...
// `owned` may be a Set of "slug:Variant" keys (preferred — pass the same set
// the UI uses so manual toggles are included) or a parsed collection.
export function encodeCode(owned, displayName) {
  const set = owned instanceof Set ? owned : ownedKeySet(owned);
  const bytes = new Uint8Array(2 + BITMAP_BYTES);
  const ck = catalogChecksum();
  bytes[0] = ck & 0xff;
  bytes[1] = ck >> 8;
  ALL_KEYS.forEach((key, i) => {
    if (set.has(key)) bytes[2 + (i >> 3)] |= 1 << (i & 7);
  });
