...
export function buildCollection(items) {
  ...
  for (const [chain, steps] of Object.entries(chainSteps)) {
    ...
    if (slug) mastery[slug] = Math.max(mastery[slug] || 0, steps.size);
    else if (steps.size > 0) unknownChains[chain] = steps.size;
  }

  return { variants, mastery, unknownChains, unmapped };
}
