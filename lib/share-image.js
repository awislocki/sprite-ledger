...
// entirely client-side. Canvas primitives (and the CORS note) live in
// share-canvas.js, shared with the trade-round renderer.
...
// keys: ["slug:Variant", ...] to include. `code` (the sender's collection
// code) is stamped into the PNG metadata so the image can be uploaded to a
// trade room and decode back to the exact collection. Returns a PNG Blob.
export async function renderShareImage({ title, subtitle, keys, code }) {
  ...
  await loadDisplayFont(["700 30px", "600 20px", "600 16px", "700 11px"]);
  ...
  return stampCode(await toPngBlob(canvas), code);
}
