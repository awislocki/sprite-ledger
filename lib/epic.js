...
// Trade a token from one client for a token on another client (same account,
// no re-login). The device-code client (switch) can authenticate the user
// but lacks the `account:public:account:deviceAuths CREATE` permission, so we
// exchange to the android client — which has it — before minting the device
// auth. Standard Epic exchange-code handoff.
export async function exchangeToClient(accessToken, targetClientKey) {
  // 1. Mint a one-time exchange code from the current token.
  const ex = await epicFetch(`${AUTH_HOST}/account/api/oauth/exchange`, {
    method: "GET",
    headers: { Authorization: `bearer ${accessToken}` },
  });
  if (!ex.res.ok)
    throw new EpicError(
      friendly(ex.data, "Couldn't finish signing in with Epic."),
      { code: ex.data?.errorCode }
    );

  // 2. Redeem it as the target client to get that client's user token.
  const { res, data } = await epicFetch(`${AUTH_HOST}/account/api/oauth/token`, {
    method: "POST",
    headers: {
      Authorization: `basic ${basicFor(targetClientKey)}`,
      "Content-Type": "application/x-www-form-urlencoded",
    },
    body: new URLSearchParams({
      grant_type: "exchange_code",
      exchange_code: ex.data.code,
    }),
  });
  if (!res.ok)
    throw new EpicError(
      friendly(data, "Couldn't finish signing in with Epic."),
      { code: data?.errorCode }
    );
  return data; // target-client user token { access_token, account_id, displayName }
}
